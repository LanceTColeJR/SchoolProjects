...
int main(void)
{
    ...
    while(com.name() != "exit")
    {
        ...
        if (com.name() == "cd") {
                    ...
                    // We actually have to account for args that aren't spotted (such as <, >, |)
                    ...
                    for (int i = 0; i < com.numArgs(); i++) {
                        // Should iterate through the command's args and add them to our char argList because that is what needed to pass through execvp()
                    ...
                    // check pid and making sure program runs in background
                    if (!com.pipeOut()) {
                        if (com.backgrounded()) {
                            int pid = fork();
                            if (pid != 0) {
