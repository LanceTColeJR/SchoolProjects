...
//define sem and num of ops for each plane
#define NUM_SEMS 1
#define NUM_OPS 1
...
//Void function for plane operation
...
int main(int argc, char *argv[])
{
    // check arg count
        ...
        semIDP = semget(2, NUM_SEMS, IPC_CREAT|IPC_EXCL|0666);

        if (semIDP < 0)
            semIDP = semget(2, NUM_SEMS, 0);
        
        semIDR = semget(1, NUM_SEMS, IPC_CREAT|IPC_EXCL|0666); // for runway

        if (semIDR < 0)
            semIDR = semget(1,  NUM_SEMS, 0);
